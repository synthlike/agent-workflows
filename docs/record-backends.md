...
Configuration rejects a routed backend unless its declared capabilities include every common record operation, the routed record type, and—when it receives `issues`—the complete issue extension. Generated helpers assume the workflow has already resolved an enabled route and obtained approval; they intentionally do not discover configuration or enforce conversational approval themselves. Workflows must not invoke a mutation helper for a disabled route without new approval.
