...
5. Accept or change the recommended named backends and all twelve semantic record routes. Provider inspection occurs only for considered backends. When selecting GitHub, choose an authenticated account explicitly, then review its capability preflight and complete managed-label plan. When selecting Bear for non-issue records, choose an absolute `bearcli` command and project workspace, then review its read-only scoped preflight.
6. Review the complete canonical plan, including exact generated bytes and hashes, prior state, directory intent, and digest. Approve that exact digest once for local configuration and guidance.
7. Let installed `apply-consumer` apply and [verify the consumer installation](verifying-installation.md). Stale state requires replanning rather than manual repair. GitHub label application, when relevant, remains a separate approval.
