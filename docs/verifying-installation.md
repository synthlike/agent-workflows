...
A schema-2 consumer verifies itself with the lifecycle command carried by its installed `configure-workflows` skill. Pass the consumer root plus the exact skill directories discovered by its harness:
...
- configuration schema, immutable distribution identity, supported issue backend, explicit GitHub login, matching backend guidance, required GitHub helper, artifact settings, repository-contained paths, or required guidance are invalid; or
