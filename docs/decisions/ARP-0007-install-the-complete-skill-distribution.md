...
A supported installation contains the complete Agent Workflows skill distribution. Users install it through an external installer or an intact manual copy at repository-contained paths discovered by their harness. `configure-workflows` records explicitly selected workflow intent separately from the complete installed inventory.
...
- `configure-workflows` never creates, replaces, or removes a skill directory.
