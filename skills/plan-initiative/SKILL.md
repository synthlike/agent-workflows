...
The map indexes answers; the detailed answer lives in exactly one resolved ticket. Refer to tickets by linked title, not bare identifier.
...
4. Create the map using [the map template](references/map-template.md).
...
2. Use a ticket named by the user, or select the first frontier ticket according to the backend.
3. Claim it as the first write.
4. Read related detail only as needed and invoke the ticket's workflow.
5. Record the answer in the ticket and resolve it.
6. Append a linked one-line gist to `Decisions so far`.
